...
server <- function(input, output, session) {
  ...
  daily_orchard2 <- ceeds::get_daily(orchard_2015)
  daily_whately2 <- ceeds::get_daily(whately_2015)
...
output$plot1 <- renderPlotly({
 ...
 v2<-reactive({
    switch(input$v,
           names(ceeds::get_daily(whately_2015)))
    })
...
# download raw data 
  ...
  output$data_table <- DT::renderDataTable({
    ...
    content = function(file) {
      write.csv(datasetInput2()[, input$columns, drop = FALSE], file)
    })
